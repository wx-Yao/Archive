import os
from Crypto.Signature import PKCS1_PSS as pss
from Crypto.Hash import SHA256
from Crypto.PublicKey import RSA
from Crypto.Cipher import AES
from Crypto.Cipher import PKCS1_OAEP as pkcsoa
from Crypto import Random

# Instead of storing files on disk,
# we'll save them in memory for simplicity
filestore = {}
# Valuable data to be sent to the botmaster
valuables = []

###

def save_valuable(data):
    valuables.append(data)

def encrypt_for_master(data):
    assert isinstance(data,bytes)
    # Encrypt the file so it can only be read by the bot master
    # We encrypt a random AES key with master's pub key and use that to encrypt files
    # As long as the pub key is correct, only the master can decrypt files
    with open("masterkey.pub",'rb') as f:
        pub=RSA.importKey(f.read())

    # We use AES-256 here
    IV=Random.new().read(AES.block_size)
    # 32 bytes key
    key=Random.new().read(32)
    cipher=AES.new(key,AES.MODE_CFB,IV)
    encrypted_data=cipher.encrypt(data)

    # Encrypt AES key now. We use PKCS#1 OAEP encryption protocol instead of naive RSA encryption scheme to avoid padding oracle attack
    # Note the OAEP doesn't require strong collision resistance of its underlying hash function hence the default SHA-1 shall be secure enough
    # Refer to reference in the report
    cipher=pkcsoa.new(pub)
    encrypted_key=cipher.encrypt(key)
    
    # Concatenate
    # Structure: IV(16 bytes)+ enc_key(512 bytes)+enc_data
    data=b"".join([IV,encrypted_key,encrypted_data])
    return data

def upload_valuables_to_pastebot(fn):
    # Encrypt the valuables so only the bot master can read them
    valuable_data = "\n".join(valuables)
    valuable_data = bytes(valuable_data, "ascii")
    print("valuable_data: {}".format(valuable_data))
    encrypted_master = encrypt_for_master(valuable_data)

    # "Upload" it to pastebot (i.e. save in pastebot folder)
    f = open(os.path.join("pastebot.net", fn), "wb")
    f.write(encrypted_master)
    f.close()

    print("Saved valuables to pastebot.net/%s for the botnet master" % fn)

###

def verify_file(f):
    assert isinstance(f,bytes)
    # We choose key length of 4096 bits hence the leading 512 bytes of the content is signature
    signature=f[:512]
    content=f[512:]
    mac=SHA256.new()
    mac.update(content)

    # We also assume that each bot can trivially obtain the master's pub key.
    with open("masterkey.pub",'rb') as f:
        pub=RSA.importKey(f.read())

    verifier=pss.new(pub)
    try:
        if verifier.verify(mac,signature):
            return True
        else:
            return False
    except ValueError:
        # trying to download encrypted but not signed file will go here
        return False

def process_file(fn, f):
    if verify_file(f):
        # If it was, store it unmodified
        # (so it can be sent to other bots)
        # Decrypt and run the file
        filestore[fn] = f
        print("Stored the received file as %s" % fn)
    else:
        print("The file has not been signed by the botnet master")

def download_from_pastebot(fn):
    # "Download" the file from pastebot.net
    # (i.e. pretend we are and grab it from disk)
    # Open the file as bytes and load into memory
    if not os.path.exists(os.path.join("pastebot.net", fn)):
        print("The given file doesn't exist on pastebot.net")
        return
    data = open(os.path.join("pastebot.net", fn), "rb").read()
    process_file(fn, data)

def p2p_download_file(sconn):
    # Download the file from the other bot
    fn = str(sconn.recv(), "ascii")
    f = sconn.recv()
    print("Receiving %s via P2P" % fn)
    process_file(fn, f)

###

def p2p_upload_file(sconn, fn):
    # Grab the file and upload it to the other bot
    # You don't need to encrypt it only files signed
    # by the botnet master should be accepted
    # (and your bot shouldn't be able to sign like that!)
    if fn not in filestore:
        print("That file doesn't exist in the botnet's filestore")
        return
    print("Sending %s via P2P" % fn)
    sconn.send(bytes(fn, 'ascii'))
    sconn.send(bytes(filestore[fn]))

def run_file(f):
    # If the file can be run,
    # run the commands
    pass
